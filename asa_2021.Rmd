...
# names(acs19_hh_income)
# names(acs19_hh_income_median)

# join tables for county name and median income
acs19_hh_income_name = inner_join(
    acs19,
    acs19_hh_income_median,
    by="GEO_ID"
)
# names(acs19_hh_income_name)

# subset and sort by median income
# income_name_vars = c("NAME", "hh_median")
# acs19_hh_income_name_only = acs19_hh_income_name[income_name_vars]
acs19_hh_income_name_only = subset(
    acs19_hh_income_name,
    as.numeric(as.character(hh_median)) >= quantile(as.numeric(as.character(hh_median)), 0.8),
    select=c(NAME, hh_median)
)
# arrange(acs19_hh_income_name_only, as.numeric(as.character(hh_median)))
acs19_hh_income_name_only %>% arrange(desc(hh_median))
