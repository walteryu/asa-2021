...
# note: fit to log scale
# source: https://stats.stackexchange.com/questions/176595/simple-log-regression-model-in-r
fit_acs19_housing_3000 <- lm(log(hh_median) ~ log(housing_more_3000),
    data=acs19_nofactor
)
round(summary(fit_acs19_housing_3000)$coeff, 6)

fit_acs19_housing_2500 <- lm(log(hh_median) ~ log(housing_more_3000) +
    log(housing_2500),
    data=acs19_nofactor
)
round(summary(fit_acs19_housing_2500)$coeff, 6)

fit_acs19_housing_2000 <- lm(log(hh_median) ~ log(housing_more_3000) +
    log(housing_2500) +
    log(housing_2000),
    data=acs19_nofactor
)
...
# note: fit to log scale
# source: https://stats.stackexchange.com/questions/176595/simple-log-regression-model-in-r
fit_acs19_edu_doctorate <- lm(log(hh_median) ~ log(edu_doctorate), data=acs19_nofactor)
round(summary(fit_acs19_edu_doctorate)$coeff, 6)

fit_acs19_edu_professional <- lm(log(hh_median) ~ log(edu_doctorate) +
    log(edu_professional),
    data=acs19_nofactor
)
round(summary(fit_acs19_edu_professional)$coeff, 6)

fit_acs19_edu_master <- lm(log(hh_median) ~ log(edu_doctorate) +
    log(edu_professional) +
    log(edu_master),
    data=acs19_nofactor
)
...
fit_acs19_edu_bachelor <- lm(log(hh_median) ~ log(edu_doctorate) +
    log(edu_professional) +
    log(edu_master) +
    log(edu_bachelor),
    data=acs19_nofactor
...
fit_acs19_best_features = lm(log(hh_median) ~ log(as.numeric(housing_more_3000)) +
...
### 2019 ACS - Model Refinement (Continued)

> Improve linear model with best features and analyze results.

1. PCA and Variance Plot
2. Logistic Regression for Median Income and Location
...
2. Histogram to Show Skew
3. Box Plot for All Counties
