...
# process data features for model fit
# acs19 = mutate(acs19,
#                # recenter pp so that intercept crosses at origin
#                # pp = (PetrolPrice - mean(PetrolPrice)) / sd(PetrolPrice),
#                # relevel factor into four brackets
#                # ppf = as.factor((pp <= -1.5) + (pp <= 0) + (pp <= 1.5) + (pp < Inf)),
#                # scale kms so that it is in proportion to other vars
#                # mm = kms / 1000,
#                hh_median_1k = hh_median / 1000,
#                poverty_below_1k = poverty_below / 1000
#                # recenter mm so that intercept crosses at origin
#                # mmc = mm - mean(mm)
#                )

# linear regression model fit - poverty
print("Fit model and output summary:")
fit_acs19_poverty <- lm(hh_median ~ poverty_below, data=acs19_nofactor)
round(summary(fit_acs19_poverty)$coeff, 4)
